...
const SortableTableRow: React.FC<SortableItemProps> = ({ id, children }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
const CloudStorage = () => {
  ...
  const handleStorageConfigUpdate = async () => {
    try {
      if (typeof window !== 'undefined' && window.electron) {
        await window.electron.updateStorageConfig({ total: newStorageSize });
        const response = await window.electron.getStorageConfig();
        setStorageTotal(response.total);
        setStorageUsed(response.used);
        setIsStorageConfigOpen(false);
  ...
  const getCurrentViewFiles = () => {
    // If we're inside a folder, show only files from that folder
    if (currentFolderId) {
      return files.filter(file => file.parentId === currentFolderId);
    }
    
    // Otherwise show root level files (files with no parentId)
            ...
            <Table>
              ...
              <TableBody>
                {paginatedFiles.map((file) => {
                  ...
                  return (
                        <SortableTableRow key={file.id} id={file.id}>
                          {(listeners) => (
                            <>
                      ...
                      {selectionMode && (
                        <TableCell>
                          ...
                          />
                              ...
                              <TableCell className="font-medium" onClick={() => file.type === 'folder' && handleFolderClick(file.id, file.name)}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {paginatedFiles.map((file) => {
                ...
                return (
                  ...
                  >
                            <div 
                              className="absolute top-2 right-2 z-10 cursor-grab"
                              {...listeners}
                            >
                              <GripVertical className="h-4 w-4 text-muted-foreground" />
                      </div>
